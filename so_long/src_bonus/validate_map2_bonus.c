...
/*   Updated: 2024/03/18 22:59:40 by srudman          ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
{
	if (map->matrix[y][x] == '1' || map->matrix[y][x] == 'X') 
		return ;
	...
	if (map->matrix[y][x] == 'C')
		map->curr_score++;
	map->matrix[y][x] = '1';
	if (map->matrix[y + 1][x] != '1' && map->matrix[y + 1][x] != 'X')
		ft_flood_fill(x, y + 1, map);
	if (map->matrix[y - 1][x] != '1' && map->matrix[y - 1][x] != 'X')
		ft_flood_fill(x, y - 1, map);
	if (map->matrix[y][x + 1] != '1' && map->matrix[y][x + 1] != 'X')
...
The only allowed characters are 0 1 C E P X and \n.
*/
